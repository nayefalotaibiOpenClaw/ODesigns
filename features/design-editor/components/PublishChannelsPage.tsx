              ...
              {activeAccounts.length > 0 && (
                <div className="relative">
                  <select
                    ...
                    className="bg-slate-50 border border-slate-200 rounded-full pl-7 pr-3 py-1.5 text-xs font-medium text-slate-700 focus:outline-none focus:border-slate-400 appearance-none"
                  >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {activeAccounts.map((account) => {
                      ...
                      return (
                        <div
                          key={account._id}
                          className="bg-white border border-slate-200 rounded-2xl p-4 flex items-center gap-4 group hover:border-slate-300 transition-colors"
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-bold text-slate-900 truncate">{account.providerAccountName}</p>
                            <p className="text-xs text-slate-400 capitalize">{account.provider}</p>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="w-2 h-2 rounded-full bg-green-500" />
                            <button
                              onClick={() => handleDisconnect(account._id)}
                              className="opacity-0 group-hover:opacity-100 p-1.5 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-red-500 transition-all"
                            >
                              <Trash2 size={14} />
                            </button>
                          </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        {provider.available ? (
                          ...
                          <span className="text-[10px] font-bold text-slate-400 uppercase">Soon</span>
                        )}
                      </button>
                    );
                  })}
